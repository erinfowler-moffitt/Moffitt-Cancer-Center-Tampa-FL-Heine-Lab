...
file='/data/Heine/work/Erin/Do_All/GE_Correlation/DAT/BASIC_INFORMATION_DS180_20181008.dat';;A dat file that holds IDNs (a string array with our study ID names).
restore, file

front_path = '/data/Heine/work/Erin/Do_All/GE_Correlation/IMAGES/' ;;This is a folder that we created with the .dcm images saved in gray-scale floating .tiff format the masks (0/1) images that helps us locate the breast area saved in .png format.
...
save_path = '/data/Heine/work/Erin/Do_All/GE_Correlation/DAT/' ; a new location of your choice to store the information output from the alogrithm
nfile = save_path+'PDA_SA5_Box2_20260721_GE_FFDM.dat' 
...
for i=0, n-1 do begin &$
	if file_test(rpath[i]) eq 1 and pda[i, 0, 0, 0] eq 0. then begin &$
		...
		pda[i, *, *, 1] = get_dense_FFDM_raw(sim1, full, lseg, nrsa2, sig1, sig2, var_r2, box, sa, flag) &$
		;im1 = get_dense_return_image_2025(sim1, full, nrsa1, sig1[1], sig2[1], var_r, box, sa, flag) &$ <- this program can return as the output if you want to look at a specific sigma1/sigma1 density layout;
